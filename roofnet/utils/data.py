    ...
    def __init__(self,
        ...
        for k, v in data_dict.items():
            ...
            for i in range(m):
                ...
                self.data.append(
                    (Image.fromarray(sorted_v['imgs'][i], 'RGB'), sorted_v['years'][i], transition_year, v['address']))
...
class TripletBuildingSampler(Sampler):
    ...
    def _one_batch(self):
        ...
        ids = range(roof_id*self.roofs_per_group,
                    (1+roof_id)*self.roofs_per_group)
        ...
        if trans_id > 0:
            ...
            while redundant:
                ...
                ref_id, pos_id = np.random.choice(
                    range(trans_id+1), 2, replace=False)
            ...
            ref_id, pos_id = np.random.choice(
                range(1, len(years)), 2, replace=False)
            neg_id = 0
